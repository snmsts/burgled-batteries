...
(defpytype ("PyObject" "PyBaseObject")
  (:to (value type)
    (loop :for (nil . type-parser) :in *type-map*
...
;;           PyByteArray for reliable octet-array conversion.
       ...
       ;; FIXME: we'll need the bytes.whatever functions, and a way to alias the
...
;; FIXME: check for errors in fopen return, double-check that fclose/fileno
;; error check is actually appropriate
(cffi:defctype file* :pointer "C-level FILE pointer.")
(cffi:defctype -1-on-failure/no-fetch (can-error :int :failure -1 :fetchablep nil))
(cffi:defcfun ("fopen" %fopen)   file* (path :string) (mode :string))
(cffi:defcfun ("fclose" %fclose) 0-on-success/no-fetch (file file*))
(cffi:defcfun ("fileno" %fileno) -1-on-failure/no-fetch (stream file*))

(defcstruct compiler-flags (flags :int))
#+requires-CHAR*-ARRAY-support (defpyfun "Py_Main" :int ((argc :int) (argv (:array :string)))) ;canerr
(defpyfun "PyRun_AnyFile"        0-on-success/no-fetch ((fp file*) (filename :string)))
(defpyfun "PyRun_AnyFileFlags"   0-on-success/no-fetch ((fp file*) (filename :string) (flags (:pointer (:struct compiler-flags)))))
(defpyfun "PyRun_AnyFileEx"      0-on-success/no-fetch ((fp file*) (filename :string) (closeit :int)))
(defpyfun "PyRun_AnyFileExFlags" 0-on-success/no-fetch ((fp file*) (filename :string) (closeit :int) (flags (:pointer (:struct compiler-flags)))))
...
(defpyfun "PyRun_SimpleFile"        0-on-success/no-fetch ((fp file*) (filename :string)))
...
(defpyfun "PyRun_SimpleFileEx"      0-on-success/no-fetch ((fp file*) (filename :string) (closeit :int)))
(defpyfun "PyRun_SimpleFileExFlags" 0-on-success/no-fetch ((fp file*) (filename :string) (closeit :int) (flags (:pointer (:struct compiler-flags)))))
;; FIXME: The Python docs leave me uncertain as to whether these are /no-fetch
(defpyfun "PyRun_InteractiveOne"       0-on-success ((fp file*) (filename :string)))
(defpyfun "PyRun_InteractiveOneFlags"  0-on-success ((fp file*) (filename :string) (flags (:pointer (:struct compiler-flags)))))
(defpyfun "PyRun_InteractiveLoop"      0-on-success ((fp file*) (filename :string)))
(defpyfun "PyRun_InteractiveLoopFlags" 0-on-success ((fp file*) (filename :string) (flags (:pointer (:struct compiler-flags)))))
...
(defpyfun "PyParser_SimpleParseFile"      .node ((fp file*) (filename :string) (start parser-context)))
(defpyfun "PyParser_SimpleParseFileFlags" .node ((fp file*) (filename :string) (start parser-context) (flags :int)))
...
(defpyfun "PyRun_File"        object! ((fp file*) (filename :string) (start parser-context) (globals dict) (locals dict)))
(defpyfun "PyRun_FileEx"      object! ((fp file*) (filename :string) (start parser-context) (globals dict) (locals dict) (closeit :int)))
(defpyfun "PyRun_FileFlags"   object! ((fp file*) (filename :string) (start parser-context) (globals dict) (locals dict) (flags (:pointer (:struct compiler-flags)))))
(defpyfun "PyRun_FileExFlags" object! ((fp file*) (filename :string) (start parser-context) (globals dict) (locals dict) (closeit :int) (flags (:pointer (:struct compiler-flags)))))
...
#-cffi-features:no-long-long (defpyfun "PyLong_FromLongLong"         long! ((v :long-long)))
#-cffi-features:no-long-long (defpyfun "PyLong_FromUnsignedLongLong" long! ((v :unsigned-long-long)))
...
#-cffi-features:no-long-long (defpyfun "PyLong_AsLongLong"             (soft-error :long-long)          ((pylong object)))
#-cffi-features:no-long-long (defpyfun "PyLong_AsUnsignedLongLong"     (soft-error :unsigned-long-long) ((pylong object)))
(defpyfun "PyLong_AsUnsignedLongMask"     :ulong                           ((io object)))
#-cffi-features:no-long-long (defpyfun "PyLong_AsUnsignedLongLongMask" :unsigned-long-long              ((io object)))
...
(defpyfun* unicode.from-string-and-size
    (("PyUnicode_FromStringAndSize" unicode! ((u utf8-string) (size ssize-t))))
  (:requires "Python 2.6 (or newer)"))
(defpyfun* unicode.from-string
    (("PyUnicode_FromString" unicode! ((u utf8-string))))
  (:requires "Python 2.6 (or newer)"))
(defpyfun* unicode.from-format
    (("PyUnicode_FromFormat" unicode! ((format :string) &rest)))
  (:requires "Python 2.6 (or newer)"))
#+requires-va_list-support
(defpyfun* unicode.from-format-v
    (("PyUnicode_FromFormatV" unicode! ((format :string) (vargs va_list))))
  (:requires "Python 2.6 (or newer)"))
(defpyfun* unicode.as-unicode ; WARNING!  Do NOT free returned buffer!
    (("PyUnicode_AsUnicode" (can-error ucs4-string) ((unicode object)))))
...
;;        :pointer for now because we aren't checking here and so might get it
;;        wrong it we assumed.
(defpyfun "PyUnicode_EncodeMBCS" object! ((s :pointer) (size ssize-t) (errors :string))
  (:requires "Windows"))
(defpyfun ("PyUnicode_AsMBCSString" unicode.as-mbcs-string) object!  ((unicode unicode))
  (:requires "Windows"))

;; Methods and Slots
(defpyfun* unicode.concat
    (("PyUnicode_Concat" unicode! ((left object) (right object)))))
(defpyfun* unicode.split
    (("PyUnicode_Split" list! ((s object) (sep object) (maxsplit ssize-t)))))
(defpyfun* unicode.splitlines
    (("PyUnicode_Splitlines" list! ((s object) (keepend :int)))))
(defpyfun* unicode.translate
    (("PyUnicode_Translate" unicode! ((str object) (table object) (errors :string)))))
(defpyfun* unicode.join
    (("PyUnicode_Join" unicode! ((separator object) (seq object)))))
(defpyfun* unicode.tailmatch
    (("PyUnicode_Tailmatch" boolean! ((str object) (substr object) (start ssize-t) (end ssize-t) (direction :int)))
     ))
(defpyfun* unicode.find
    (("PyUnicode_Find" ssize-t! ((str object) (substr object) (start ssize-t) (end ssize-t) (direction :int)))
     ))
(defpyfun* unicode.count
    (("PyUnicode_Count" ssize-t! ((str object) (substr object) (start ssize-t) (end ssize-t)))
     ))
(defpyfun* unicode.replace
    (("PyUnicode_Replace" unicode! ((str object) (substr object) (replstr object) (maxcount ssize-t)))
     ))
(defpyfun* unicode.compare
    (("PyUnicode_Compare" :int ((left object) (right object)))))
(defpyfun* unicode.rich-compare
    (("PyUnicode_RichCompare" object! ((left object) (right object) (op :int)))))
(defpyfun* unicode.format
    (("PyUnicode_Format" unicode! ((format object) (args object)))))
(defpyfun* unicode.contains
    (("PyUnicode_Contains" boolean! ((container object) (element object)))))
